    ...
    <div className="flex gap-1">
      {String(value)
        ...
        .map((digit, i) => (
          <div
            key={i}
            className="w-8 h-10 bg-red-500/10 border border-red-500/20 rounded-lg flex items-center justify-center text-base font-black text-red-600 shadow-sm"
          >
            {digit}
          </div>
        ))}
...
export default function MockTestPage() {
  ...
  const categorizeTests = (tests: any[]) => {
    ...
    tests.forEach((test: any) => {
      ...
      const transferEnd = addMinutes(
        start,
        test.live_to_previous_minutes || 180
      );
      ...
      if (secondsUntilStart > 0) {
        ...
        upcoming.push({
          ...test,
          mock_start_time: test.scheduled_at,
          mock_end_time: transferEnd.toISOString(),
          registration_deadline: test.scheduled_at,
          isPreStart,
        });
        ...
        live.push({
          ...
          mock_start_time: test.scheduled_at,
          mock_end_time: transferEnd.toISOString(),
          isJoinable: secondsSinceStart <= 180,
        });
      }
    });
      ...
      try {
        ...
        if (session?.user) {
          ...
          if (!controller.signal.aborted) {
            ...
            const hasGlobalAccess = purchases.some(
              (p: PurchaseRow) =>
                [
                  "practice_all",
                  "mock_all",
                  ...
                  "full_package",
                  "premium_all",
                ].includes(p.item_type) && p.status === "completed"
            );

            const purchaseRegs: Registration[] = purchases
              .filter((p: PurchaseRow) =>
                [
                  "mock_test",
                  "mock",
                  "mock_single",
              ...
              .map((p: PurchaseRow) => ({
                test_id: p.item_id ?? "", // if null, fallback to ""
                status: p.status as RegistrationStatus,
              }))
              .filter((r) => r.test_id !== ""); // remove invalid rows

            const combinedRegs: Registration[] = [
              ...((regsRes.data || []) as Registration[]),
              ...purchaseRegs,
            ];

            if (
              hasGlobalAccess &&
              !combinedRegs.some((r: Registration) => r.test_id === "GLOBAL")
            ) {
  ...
  const isRegistered = (testId: string) => {
    return registrations.some(
      (r) => r.test_id === testId || r.test_id === "GLOBAL"
    );
  };
      ...
      <div className="container max-w-7xl mx-auto px-4 py-12 md:py-20">
        <div className="grid lg:grid-cols-12 gap-12">
          <div className="lg:col-span-8">
            <div className="mb-16">
              <p className="text-xs font-black text-orange-500 mb-4 uppercase tracking-[0.25em]">
                লাইভ পরীক্ষা
              </p>
              ...
              <p className="text-lg text-muted-foreground max-w-2xl">
                সপ্তাহে ৫ দিন নির্ধারিত সময়ে সবাই একসাথে পরীক্ষা দিন। র‌্যাংকিং
                ও ফিডব্যাক পান।
              </p>
            </div>
                ...
                <div className="grid gap-6">
                  {liveTests.map((test) => {
                    ...
                    return (
                      <div
                        key={test.id}
                        className="group relative overflow-hidden rounded-[2.5rem] bg-[#0A0A0A] border border-red-500/20 shadow-[0_30px_60px_-15px_rgba(220,38,38,0.2)] transition-all duration-500 hover:scale-[1.005]"
                      >
                        ...
                        <div className="relative p-6 sm:p-8 flex flex-col md:flex-row items-center gap-8 z-10">
                          <div className="flex-1 text-center md:text-left">
                            ...
                            <h3 className="text-2xl sm:text-3xl font-black mb-3 tracking-tight text-white">
                              {test.title}
                            </h3>
                            <p className="text-sm text-white/50 leading-relaxed max-w-xl line-clamp-1">
                              {test.description}
                            </p>
                              ...
                              <div className="flex flex-col items-center gap-4">
                                <div className="flex flex-col items-center">
                                  ...
                                  <div className="flex gap-2 items-center">
                                    <CountdownBox
                                      ...
                                      label="মিনিট"
                                    />
                                    <div className="text-red-500/30 font-black text-xl mb-4">
                                      :
                                    </div>
                                    <CountdownBox
                                      value={joinSeconds}
                                ...
                                {registered ? (
                                  <Button
                                    asChild
                                    size="lg"
                                    className="w-full md:w-64 h-16 font-black rounded-2xl bg-[#74b602] hover:bg-[#86d102] text-black shadow-lg shadow-primary/20 group/btn transition-all text-lg border-none"
                                  >
                                    <button
                                      type="button"
                                  ...
                                  <div className="text-center p-4 rounded-2xl bg-red-500/10 border border-red-500/20 w-full md:w-64">
                                    ...
                                    <Button
                                      asChild
                                      variant="link"
                                      className="text-[#74b602] font-black p-0 h-auto"
                                    >
                                      <Link
                                        href="/mock/pre-book"
            ...
            {upcomingTests.length === 0 && liveTests.length === 0 && (
              <div className="mb-16 p-10 rounded-3xl border-2 border-dashed border-primary/20 bg-primary/[0.02] text-center">
                ...
                <h2 className="text-2xl font-black mb-3 text-foreground">
                  বর্তমানে কোনো লাইভ মক টেস্ট নেই
                </h2>
                <p className="text-muted-foreground mb-8 max-w-md mx-auto">
                  শীঘ্রই নতুন মক টেস্ট শুরু হবে। এখন প্রি-বুক করলে পরবর্তী মক
                  টেস্টে পাবেন সরাসরি ডিসকাউন্ট!
                </p>
                ...
                <div className="grid sm:grid-cols-2 gap-8">
                  {upcomingTests.map((test) => {
                    ...
                    return (
                      <div
                        key={test.id}
                        className="group relative flex flex-col h-full rounded-[2.5rem] bg-card border border-border/40 hover:border-primary/40 transition-all duration-500 hover:shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)] dark:hover:shadow-[0_20px_50px_-12px_rgba(116,182,2,0.15)] overflow-hidden"
                      >
                        <div
                          className={`h-1.5 w-full ${registered ? "bg-emerald-500" : "bg-primary/20 group-hover:bg-primary transition-colors"}`}
                        ></div>
                        ...
                        <div className="p-8 flex flex-col h-full">
                          <div className="flex items-center justify-between mb-8">
                            <Badge
                              variant="secondary"
                              className={`${status.color} bg-secondary/50 font-black uppercase text-[10px] tracking-[0.15em] px-4 py-1.5 rounded-xl border border-border/50`}
                            >
                              {status.text}
                            </Badge>
                          ...
                          <h3 className="text-2xl font-black mb-4 tracking-tight leading-tight group-hover:text-primary transition-colors">
                            {test.title}
                          </h3>
                            ...
                            {test.isPreStart ? (
                              <div className="flex flex-col items-center gap-3 p-4 rounded-2xl bg-orange-500/5 border border-orange-500/20 animate-in fade-in slide-in-from-bottom-2 duration-500">
                                ...
                                <div className="flex gap-2">
                                  ...
                                  <div className="text-orange-500/30 font-black text-xl mb-4">
                                    :
                                  </div>
                          ...
                          {registered ? (
                            <div className="w-full">
                              {test.isPreStart ? (
                                <Button
                                  disabled
                                  className="w-full h-14 font-black rounded-2xl bg-orange-500/10 text-orange-500 border border-orange-500/20 cursor-not-allowed"
                                >
                                  অপেক্ষা করুন...
                                </Button>
                              ) : (
                            ...
                            <Button
                              asChild
                              className="w-full h-14 font-black rounded-2xl shadow-xl shadow-primary/20 bg-primary hover:bg-primary/90 text-black border-none hover:scale-[1.02] active:scale-[0.98] transition-all text-base"
                            >
                              <Link
                                href={`/checkout/mock/${test.id}`}
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                  {pastTests.map((test) => {
                    ...
                    return (
                      <div
                        key={test.id}
                        className="p-6 rounded-[2rem] border border-border/30 bg-secondary/10 hover:bg-secondary/20 transition-all group"
                      >
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <div>
                            <h3 className="font-black group-hover:text-primary transition-colors line-clamp-1">
                              {test.title}
                            </h3>
                        ...
                        <div className="grid grid-cols-2 gap-3 mt-6">
                          <Button
                            ...
                            variant="outline"
                            size="sm"
                            className="font-black rounded-xl h-11 border-border/50 bg-background/50 hover:bg-white dark:hover:bg-white/5"
                          >
                            <Link
                              href={`/mock/${test.slug || test.id}/leaderboard`}
                            >
                              লিডারবোর্ড
                            </Link>
                          </Button>
                        ...
                        {!registered && (
                          <div className="mt-4 pt-4 border-t border-border/20">
                            <p className="text-[10px] text-center font-bold text-muted-foreground/60 flex items-center justify-center gap-2 uppercase tracking-widest">
                              <Lock className="h-3 w-3" /> শুধু রেজিস্টার্ড
                              ইউজারদের জন্য
                            </p>
                          </div>
                        )}
                ...
                {[
                  {
                    ...
                    desc: "সবাই নির্ধারিত সময়ে",
                  },
                  {
                    icon: <Trophy className="h-5 w-5" />,
                    title: "র‌্যাংকিং",
                    desc: "অন্যদের সাথে তুলনা",
                  },
                  {
                    icon: <Target className="h-5 w-5" />,
                    title: "বিস্তারিত ফিডব্যাক",
                    desc: "দুর্বলতা চিহ্নিত",
                  },
                ].map((item, i) => (
